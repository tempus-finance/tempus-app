import Decimal from '../datastructures/Decimal';
...
export class DecimalUtils {
  ...
  static formatWithMultiplier(value: number | string | Decimal, fractionDigits = 0): string {
  ...
  // TODO - If we pass in a BigNumber that does not have 18 decimals precision, this will break
  static formatToCurrency(value: number | string | Decimal, fractionDigits = 2, symbol = ''): string {
    const decimal = new Decimal(value);
  ...
  // TODO - If we pass in a BigNumber that does not have 18 decimals precision, this will break
  static formatPercentage(value: number | string | Decimal, fractionDigits = 2, roundValue = false): string {
    const decimal = new Decimal(value);
