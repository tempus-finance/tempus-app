...
const ROOT_PATH = '../'
const { LOAD_TIMEOUT, LOAD_SHORT_TIMEOUT, LOAD_LONG_TIMEOUT } = require(`${ROOT_PATH}utility/config.json`)
const {tempusManageCurrency} = require(`${ROOT_PATH}utility/tempushome.js`);

async function manageDeposit(browser, amount='1', asset='USDC', token='USDC', fixedYield=true){
    const tabManage = tempusManageCurrency(browser, asset)
